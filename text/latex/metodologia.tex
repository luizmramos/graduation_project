...
\subsection{Estudo dos resultados obtidos no conjunto de validação para diferentes características e parâmetros}
\label{sec:validacao}

Foi realizado um estudo detalhado dos resultados obtidos pelos classificadores para diferentes características, parâmetros e técnicas de classificação (NB e NB com pesos). 
...
Assim, foi feito também um classificador de artigos nos mesmos moldes do classificador de posts, utilizando \emph{Naïve Bayes} e algumas otimizações como \emph{Stemming} e \emph{Weighting}. A ideia é que quando o classificador de posts se deparasse com um link iria chamar o classificador de artigos, que classificaria o artigo contido no link em determinada classe (não necessariamente igual a alguma das classes do classificador de post) e devolveria esse resultado em forma de característica para o classificador de post, como uma forma de pré-processamento do texto.
...
Muitas vezes as palavras puras não são muito boas para utilizar como características na classificação. Por isso para cada palavra associa-se um token. Este processo é chamado de tokenização. Deve-se decidir o que será feito com a pontuação, se palavras serão modificadas, etc. As vezes um token pode ser um conjunto de palavras. Por exemplo `Rio de Janeiro' pode ser um único token.
